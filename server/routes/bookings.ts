...
export const handleGetBookingDetails: RequestHandler = async (req, res) => {
  try {
    ...
    const authResult = await authService.authenticateByBookingId(requestingBookingId);
    if (!authResult.success || !authResult.user) {
      return res.status(401).json({
        success: false,
        error: 'Authentication required'
...
export const handleUpdateBookingStatus: RequestHandler = async (req, res) => {
  try {
    ...
    const updateResult = await authService.updateBookingStatus(bookingId, status);
...
function getCoordinatorInfo(coordinatorId: string) {
  ...
  return coordinators[coordinatorId] || {
    id: coordinatorId,
