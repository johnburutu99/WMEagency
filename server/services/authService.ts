...
// Validation schemas
export const BookingIdSchema = z.string().regex(/^[A-Z0-9]{8}$/, 'Booking ID must be 8 alphanumeric characters');
...
class BookingDatabase {
  ...
  async updateBooking(bookingId: string, updates: Partial<UserData>): Promise<UserData | null> {
  ...
  private generateBookingId(): string {
    ...
    throw new Error('Unable to generate unique booking ID after maximum attempts');
  }
...
export class AuthService {
  ...
  async authenticateByBookingId(bookingId: string): Promise<{ success: boolean; user?: UserData; error?: string }> {
    try {
      ...
      if (!validation.success) {
        return {
          success: false,
          error: 'Invalid booking ID format. Must be 8 alphanumeric characters.'
        };
      }
      ...
      if (!user) {
        return {
          success: false,
          error: 'Booking ID not found. Please check your booking confirmation.'
        };
      }
      ...
      if (user.status === 'cancelled') {
        return {
          success: false,
          error: 'This booking has been cancelled. Please contact your coordinator.'
        };
      }
  ...
  async createNewBooking(data: CreateBookingRequest): Promise<{ success: boolean; booking?: UserData; error?: string }> {
    try {
      // Validate input data
      if (!data.clientName || !data.clientEmail || !data.artist || !data.event) {
        return {
          success: false,
  ...
  async updateBookingStatus(bookingId: string, status: UserData['status']): Promise<{ success: boolean; booking?: UserData; error?: string }> {
    try {
